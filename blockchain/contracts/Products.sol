...
interface ITrade {
    function getBuyerAdr(uint256 tokenId) external view returns (address);
    function tradeEnd(uint256 tokenId) external;
    function msgSenderTest() external view returns (address);
}
...
contract Products is KIP17Full, KIP17Mintable, KIP17MetadataMintable, KIP17Burnable, KIP17Pausable {
    using SafeMath for uint256;
    
    ITrade _tradeCA;
    
    event ProductEnrolled (uint256 indexed tokenId, uint256 salePrice, string productName, string size, bool isSell);
    
    mapping (uint256 => ProductData) public productList;
    ...
    function setTradeContract(ITrade adr) public onlyMinter {
        _tradeCA = adr;
    }
    
    function enrollProduct(address seller, uint256 salePrice, string memory productName, string memory size) public onlyMinter {
        uint256 tokenId = totalSupply().add(1);
        _mint(seller, tokenId);
        
        bool isSell = true;
        uint256[] memory soldPriceHistory;
        address[] memory ownerHistory;
        
        ProductData memory newProductData = ProductData({
            tokenId: tokenId,
            salePrice: salePrice,
        ...
        isPaid[tokenId] = false;
        productList[tokenId] = newProductData;
        productList[tokenId].ownerHistory.push(seller);
        
        emit ProductEnrolled(tokenId, salePrice, productName, size, isSell);
    }
    
    function getProduct(uint tokenId) public view returns (uint256, uint256, string memory, string memory, bool, uint256[] memory, address[] memory) {
        require(productList[tokenId].tokenId != 0, "Product does not exist");
        return (
            productList[tokenId].tokenId,
            productList[tokenId].salePrice,
            productList[tokenId].productName,
            productList[tokenId].size,
            productList[tokenId].isSell,
            productList[tokenId].soldPriceHistory,
            productList[tokenId].ownerHistory
        );
    }
    
    function confirmTrade(uint256 tokenId) public returns (bool) {
        require(productList[tokenId].isSell == true);
        require(isPaid[tokenId] == true);
        productList[tokenId].isSell = false;
        address buyer = _tradeCA.getBuyerAdr(tokenId);
        _tradeCA.tradeEnd(tokenId);
        transferToken17(buyer, tokenId);
        return true;
    }
    
    function transferToken17(address to, uint256 tokenId) public {
        uint256 soldPrice = productList[tokenId].salePrice;
        safeTransferFrom(msg.sender, to, tokenId);
        productList[tokenId].isSell = false;
        productList[tokenId].soldPriceHistory.push(soldPrice);
        productList[tokenId].ownerHistory.push(to);
        productList[tokenId].salePrice = 0;
    }
    
     function setIsPaidTrue(uint256 tokenId) public {
        isPaid[tokenId] = true;
    }
    
    function setIsPaidFalse(uint256 tokenId) public {
        isPaid[tokenId] = false;
    }
    
    function getSalePrice(uint256 tokenId) public view returns (uint256) {
        require(productList[tokenId].tokenId != 0, "Product does not exist");
        return productList[tokenId].salePrice;
    }
    
    function getIsSell(uint256 tokenId) public view returns (bool) {
        require(productList[tokenId].tokenId != 0, "Product does not exist");
        return productList[tokenId].isSell;
    }
    
    function getIsPaid(uint256 tokenId) public view returns (bool) {
        require(productList[tokenId].tokenId != 0, "Product does not exist");
        return isPaid[tokenId];
    }
    
    function msgSenderCall() public view returns (address) {
        return _tradeCA.msgSenderTest();
    }
}
